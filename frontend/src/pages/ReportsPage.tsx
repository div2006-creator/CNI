        ...
        <div className="border-b border-[#e5dfd3] pb-6 flex flex-wrap justify-between items-start gap-4">
          <div>
            ...
            <h1 className="text-xl md:text-2xl font-bold text-stone-900 uppercase tracking-tight mt-3">
              {meta.case_title || 'Active Executive Intelligence Dossier'}
            </h1>
            <p className="text-xs font-mono text-stone-600 mt-1">
              CASE NUMBER: <strong className="text-stone-900">{meta.case_id || 'INV-2026-0000'}</strong> | SYSTEM VERSION: {meta.system_version || 'CrimeNet v2.4'}
            </p>
          </div>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className="p-4 bg-[#f8f6f0] border border-[#e5dfd3] rounded-xl space-y-1">
                ...
                <div className="text-2xl font-bold font-mono text-stone-900">{exec.total_entities_analyzed ?? 0}</div>
              </div>
              <div className="p-4 bg-[#f8f6f0] border border-[#e5dfd3] rounded-xl space-y-1">
                <span className="text-xs text-stone-600 font-mono">Relationships Modeled</span>
                <div className="text-2xl font-bold font-mono text-stone-900">{exec.total_relationships_modeled ?? 0}</div>
              </div>
              <div className="p-4 bg-[#f8f6f0] border border-[#e5dfd3] rounded-xl space-y-1">
                <span className="text-xs text-stone-600 font-mono">Critical Risk Targets</span>
                <div className="text-2xl font-bold font-mono text-rose-600">{exec.critical_risk_targets_count ?? 0}</div>
              </div>
              <div className="p-4 bg-[#f8f6f0] border border-[#e5dfd3] rounded-xl space-y-1">
                <span className="text-xs text-stone-600 font-mono">Bridge Entities Flagged</span>
                <div className="text-2xl font-bold font-mono text-amber-700">{exec.bridge_nodes_detected_count ?? 0}</div>
              </div>
            </div>
          </div>
        )}
            ...
            <div className="p-6 bg-[#f8f6f0] border border-[#e5dfd3] rounded-2xl space-y-6">
              <div className="flex flex-wrap items-center justify-between gap-4 border-b border-[#e5dfd3] pb-4">
                <div>
                  ...
                  <div className="text-base font-bold text-stone-900 mt-0.5">{evidence.pair_subject || 'Ingested Relationship Pair'}</div>
                </div>
                <div className="text-right">
                  <div className="text-xs font-mono text-stone-600">Explainable Confidence Score</div>
                  <div className="text-xl font-bold font-mono text-saffron-700">{evidence.confidence_percentage || 0}%</div>
                </div>
              </div>
